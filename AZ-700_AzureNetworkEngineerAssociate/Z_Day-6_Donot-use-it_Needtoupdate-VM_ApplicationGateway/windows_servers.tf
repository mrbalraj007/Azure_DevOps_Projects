...
resource "azurerm_virtual_machine_extension" "rg01_vm2_extension" {
  ...
  settings = jsonencode({
    "commandToExecute" : "powershell -ExecutionPolicy Unrestricted -Command \"netsh advfirewall set allprofiles state off; Install-WindowsFeature -name Web-Server -IncludeManagementTools; New-Item -Path 'C:\\inetpub\\wwwroot\\videos' -ItemType Directory -Force; Set-Content -Path 'C:\\inetpub\\wwwroot\\videos\\index.html' -Value '<html><body><h1>This is the VIDEOS server (VM02)</h1><p>Path is working correctly through Application Gateway!</p></body></html>'; Set-Content -Path 'C:\\inetpub\\wwwroot\\index.html' -Value '<html><body><h1>This is the default page for VM02</h1><p>Go to <a href=\\\"/videos/\\\">/videos/</a> for the videos section</p></body></html>'; Write-Host 'Adding firewall rules for Application Gateway...'; New-NetFirewallRule -Name 'Allow_AppGateway_Traffic' -DisplayName 'Allow App Gateway' -Direction Inbound -Protocol TCP -LocalPort 80 -Action Allow; Write-Host 'Verifying videos directory exists...'; if (Test-Path 'C:\\inetpub\\wwwroot\\videos') { Write-Host 'Directory exists!' } else { Write-Host 'Directory does not exist!'; throw 'Failed to create videos directory' }; icacls 'C:\\inetpub\\wwwroot\\videos' /grant 'IIS_IUSRS:(OI)(CI)(RX)' /grant 'BUILTIN\\Users:(OI)(CI)(RX)'\""
  })
