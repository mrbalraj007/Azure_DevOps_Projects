...
variable "keyvault_name" {
  description = "Base name of the Azure Key Vault (will be appended with a random suffix)"
  type        = string
}

variable "sp-user_object_id" {
  description = "Object ID of a service principal who should have access to the Key Vault (leave empty if not needed)"
  type        = string
  default     = ""
}
...
variable "kv_secret_permissions" {
  description = "Permissions for secrets in the Key Vault"
  type        = list(string)
  default     = ["Get", "List", "Set", "Delete", "Recover", "Backup", "Restore", "Purge"]
}

variable "kv_key_permissions" {
  description = "Permissions for keys in the Key Vault"
  type        = list(string)
  default     = ["Get", "List", "Create", "Delete", "Update", "Import", "Backup", "Restore", "Recover"]
}

variable "kv_certificate_permissions" {
  description = "Permissions for certificates in the Key Vault"
  type        = list(string)
  default     = ["Get", "List", "Create", "Delete", "Update", "Import", "ManageContacts", "GetIssuers", "ListIssuers"]
}

variable "kv_storage_permissions" {
  description = "Permissions for storage in the Key Vault"
  type        = list(string)
  default     = ["Get", "List", "Set", "Update", "RegenerateKey", "Recover", "Delete", "Backup", "Restore", "Purge"]
}
...
variable "bastion_public_ip_name" {
  description = "Name of the public IP for Azure Bastion Host"
  type        = string
}
...
variable "bastion_ip_configuration_name" {
  description = "Name of the IP configuration for Azure Bastion Host"
  type        = string
}

variable "bastion_dns_label_prefix" {
  description = "DNS label prefix for Azure Bastion Host"
  type        = string
}