...
- stage: create_frontdoor
  
  jobs:
  - job: create_frontdoor
    displayName: 'Create Azure FrondDoor services'
    steps:
    - checkout: self
    - task: TerraformInstaller@1
      inputs:
    ...
    - task: TerraformTaskV4@4
      inputs:
        provider: 'azurerm'
        command: 'init'
        workingDirectory: '$(Build.SourcesDirectory)/front-door'
        backendServiceArm: 'Azure Service Connection'
        backendAzureRmResourceGroupName: 'rg-devops'
        backendAzureRmStorageAccountName: 'storageacctd2e215eb'
        backendAzureRmContainerName: 'tfstates'
        backendAzureRmKey: 'frontdoor.tfstate'
    - task: TerraformTaskV4@4
      inputs:
        provider: 'azurerm'
        command: 'apply'
